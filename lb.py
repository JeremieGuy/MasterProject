...
def main():
    ...
    for y in range(0,Ny):
        for x in range(0,Nx):
            if (distance(Nx//4,Ny//2,x,y)<10): # set center at half of y height and 1/4 of x distance ('//' = integer division)
                cylinder[y][x] = True
    ...
    for it in range(Nt//deltaT):
        ...
        # set boundaries
        boundaryF = f[cylinder,:] # set the nodes that are in the cylinder ... 
        ...
        # Boundary step (cylinder - 2D only)
        ...
        if (it%plottingFrames == 0):
            ...
            plt.imshow(np.sqrt(ux**2+uy**2)) # 2D
            plt.pause(.01)
            plt.cla()
